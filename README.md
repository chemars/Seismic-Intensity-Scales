# 地震震度計算
以地震觀測資料計算出中央氣象署（CWA）、日本氣象廳（JMA）、福建省地震局（FJEA）所使用的震度。

中央氣象署地震震度計算與繪圖的整合版本請參考 https://github.com/chemars/Seismic-Intensity
...
## 計算
計算程式皆為函數（function），呼叫格式為`函數名稱(觀測資料檔案名稱,分隔符號,要忽略的列數,南北向行的位置,東西向行的位置,上下向行的位置,取樣率)`，`cwa2000`無取樣率參數。`cwa2020`輸出為字串，其餘輸出為數值。
* `cwa2000`對應的震度為中央氣象署於2000年8月1日公告修訂的震度，分為0級至7級。
* `cwa2020`對應的震度為中央氣象署於2020年1月1日起施行的震度，分為0級、1級、2級、3級、4級、5弱、5強、6弱、6強、7級。
...
## 範例
地震觀測資料來源：中央氣象署 [個案地震報導](https://scweb.cwa.gov.tw/zh-tw/page/disaster/3)、日本氣象廳 [強震観測データ](https://www.data.jma.go.jp/svd/eqev/data/kyoshin/jishin/index.html)

以[中央氣象署的檔案](https://scweb.cwa.gov.tw/special/19990921/ASCIIfile/T073001.263.txt)為例，可使用下列方式計算出震度（jma 可更改為 fjea）

```octave
>> cwa2000("T073001.263.txt","",11,3,4,2)
ans =  6
>> jma("T073001.263.txt","",11,3,4,2,200)
ans =  5.2000
>> cwa2020("T073001.263.txt","",11,3,4,2,200)
ans = 6-
```
...
* [交通部中央氣象局有感地震報告發布作業要點](https://web.archive.org/web/20190511134757/https://www.cwb.gov.tw/Data/service/notice/download/notice_20141231104524.pdf)（中華民國103年12月22日中象地字第1030015380號函修正）
* [交通部中央氣象局有感地震報告發布作業要點](https://web.archive.org/web/20191211105014/https://www.cwb.gov.tw/Data/service/notice/download/notice_20191210113759.pdf)（中華民國108年12月2日中象地字第1080016036號函修正）
...
* [地震仪器烈度表](https://web.archive.org/web/20190613121823/http://www.fjdspm.com/dzzt/zcfgzt/2013-03-14/231.html)
